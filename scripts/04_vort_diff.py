...
for exp, scorrData, biasData, ax, lList in zip(exps, scorr_vorts, bias_vorts, 
    ...
    plotDA(biasData, ax, f'{lList}) {exp} - ERA5 : Rel Vort', 
           f'CC:{scorrData}', 
           normVORT, cmap=cmapVORT, 
           transform=ccrs.PlateCarree())
cbar_ax = fig.add_axes([0.775, .18, .21, .02])
plotCBAR(cbar_ax, '850-hPa Relative\nVorticity Difference\n[$\mathregular{10^{-5}}$/s]', normVORT, cmap=cmapVORT)
#---------------------------------------------------------------------------------

#---------------------------------------------------------------------------------
### save Fig
fig.savefig(dir / 'img/Fig07_vort_diff.png',
            transparent=True,
            bbox_inches='tight')
#---------------------------------------------------------------------------------