...
## ERA5 CMAP
MSLP_colors = ["#5c3c82ff", "#927dacff", "#c9bed5ff", "#e4deeaff",
                   "#f2e2deff", "#e5c4bdff", "#cc8a7aff", "#b24f38ff"]
cmapMSLP_era5 = mp.colors.ListedColormap(MSLP_colors)
cmapMSLP_era5.set_under("#2d004bff")
cmapMSLP_era5.set_over("#a82e2aff")
vmin = 1008
vmax = 1018
normMSLP_era5 = mcolors.Normalize(vmin=vmin, vmax=vmax)

WIND_colors = ["#70bcbd", "#89c6c7", "#a4d0d1", '#c0dadb',
                   "#debeaf", "#d59a9f", "#c86c8a", "#b82663"]
cmapWIND_era5 = mp.colors.ListedColormap(WIND_colors)
cmapWIND_era5.set_under("#579ca0")
cmapWIND_era5.set_over("#981d52")
vmin = 1
vmax = 7
normWIND_era5 = mcolors.Normalize(vmin=vmin, vmax=vmax)

VORT_colors = ["#542788ff", "#8073acff", "#b2abd2ff",  "#d8daebff",
                   "#fee0b6ff","#fdb863ff", "#e08214ff", "#b35806ff"]
cmapVORT_era5 = mp.colors.ListedColormap(VORT_colors)
cmapVORT_era5.set_under("#2d004bff")
cmapVORT_era5.set_over("#7f3b08ff")
vmin = -3
vmax = 3
normVORT_era5 = mcolors.Normalize(vmin=vmin, vmax=vmax)

WSHEAR_colors = ["#542788ff", "#8073acff", "#b2abd2ff",  "#d8daebff",
                    "#fee0b6ff","#fdb863ff", "#e08214ff", "#b35806ff",]
cmapWSHEAR_era5 = mp.colors.ListedColormap(WSHEAR_colors)
cmapWSHEAR_era5.set_under("#2d004bff")
cmapWSHEAR_era5.set_over("#7f3b08ff")
vmin = 1
vmax = 28
normWSHEAR_era5 = mcolors.Normalize(vmin=vmin, vmax=vmax)

TEMP_colors = ["#163976", "#2e6292", "#6695b5", "#aec8d8",
                   "#dabaa3", "#c38b67", "#ab5f33", "#7b2b14"]
cmapTEMP_era5 = mp.colors.ListedColormap(TEMP_colors)
cmapTEMP_era5.set_under("#04125d")
cmapTEMP_era5.set_over("#510b0d")
vmin = -5
vmax = -1
normTEMP_era5 = mcolors.Normalize(vmin=vmin, vmax=vmax)
...
#ERA5
axList = [axs['A'], axs['D'], axs['G'],axs['J'], axs['M']]
letters = ['a', 'b', 'c', 'd', 'e']
titles = ['MSLP', '850-hPa Wind Speed', '850-hPa Rel Vort',
          '300-850-hPa Wind Shear', 'Mean Temperature'] 
titles_cbar = ['MSLP [hPa]', 'Wind Speed[m/s]', 'Rel Vort [$\mathregular{10^{-5}}$/s]',
               'Wind Shear [m/s]', 'Mean Temp [\N{DEGREE SIGN}C]']
era5s = [ref_mslp, ref_wspd, ref_vort, ref_wshear, ref_tmean]
norms = [normMSLP_era5, normWIND_era5, normVORT_era5,
         normWSHEAR_era5, normTEMP_era5]
cmaps = [cmapMSLP_era5, cmapWIND_era5, cmapVORT_era5,
         cmapWSHEAR_era5, cmapTEMP_era5]
for era, ax, lList, title, title_cb, norm, cmap in zip(era5s, axList, letters,
                                                       titles, titles_cbar,
                                                       norms, cmaps):
    plotDA(era, ax, f'{lList}) ERA5\n{title}', '',
           norm, cmap=cmap,
           transform=ccrs.PlateCarree())
    plotCBAR(ax, title_cb, norm, cmap=cmap)

#MSLP
axList = [axs['B'], axs['C']]
letters = ['f', 'k']
descriptions = ['BEST', 'WORST']
...
for exp, scorrData, biasData, ax, lList in zip(exps, scorr_winds, bias_winds, 
                                               axList, letters):
    print(f'Plotting for {exp}')
    plotDA(biasData, ax, f'{lList}) {exp} - ERA5\n850-hPa Wind Speed Difference', 
           f'CC:{scorrData}', 
...
for exp, scorrData, biasData, ax, lList in zip(exps, scorr_wshears, bias_wshears, 
                                               axList, letters):
    print(f'Plotting for {exp}')
    plotDA(biasData, ax, f'{lList}) {exp} - ERA5\n300-850-hPa Wind Shear Difference', 
           f'CC:{scorrData}', 
...
#---------------------------------------------------------------------------------
### save Fig
fig.savefig(dir / 'img/Fig10_era5_best_worst_diff.png',
            transparent=True,
            bbox_inches='tight')
#---------------------------------------------------------------------------------
